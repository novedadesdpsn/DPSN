...
function cambiarPestanaAsistente(cual) {
  document.querySelectorAll('.asistente-tab').forEach(b => b.classList.toggle('activo', b.dataset.tab === cual));
  const cuerpo = document.getElementById('asistenteCuerpo');
  cuerpo.innerHTML = cual === 'buscar' ? renderAsistente() : renderEstadisticas();
}
